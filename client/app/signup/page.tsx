...
const SignUp = () => {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if(isValidSignUpParams(errors)){
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}/v1/signup`, {
  ...
  useEffect(() => {
    if(inputs.password){
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}/v1/password-score`, {
        password: inputs.password,
      })
      .then((response) => {
        if(response.status === 200){
          setPasswordStrength(response.data.score)
        }
      })
    }else{
